...
> **Build / 构建：**`a46b7bc9b2b3059762fce80ab7db7214e13fe1c3`。
> 包含 SIGHASH 映射、锁定竞态、多钱包管理和过期状态响应隔离修复。
> Includes explicit SIGHASH mapping, lock-race fixes, independent multi-wallet
> management and stale status-response isolation. This remains a developer
> preview; use only valueless test funds. Release asset upload and remote hash
> verification must complete before distributing this build from GitHub.
...
- 支持多个独立钱包的命名、切换，以及验证密码后重新备份助记词；原钱包不覆盖。
- 切换钱包撤销网站授权并隔离旧查询。暂不支持同一助记词下多个派生账户或删除钱包。
...
通过 56 项测试、Electron 冒烟测试及打包，但真实代币转账、第三方 dApp 全流程、
...
- Independent named wallets, switching and password-verified recovery phrase
  display. Legacy wallets are preserved. Switching revokes site permissions and
  invalidates stale queries. Multiple derived accounts and deletion are not offered.
...
**Not a production wallet.** All three OS runners passed 56 automated tests,
...
Validation evidence / 验证记录: [CI run 34330304804](https://github.com/w00c00/kaspa-orbit/actions/runs/34330304804).
This run tests `a46b7bc`, including the independent multi-wallet lifecycle and
stale-context guards. It does not prove all third-party dApp compatibility.
