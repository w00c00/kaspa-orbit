...
async function verifyCovenantCell(service,input){
 ...
 if(!Number.isSafeInteger(index)||index<0||index>0xffffffff)throw Error('Invalid output index');
 ...
 try{
  ...
  return await service.withRpc(async rpc=>{
   ...
   const entry=entries.find(e=>String(e.outpoint.transactionId).toLowerCase()===transactionId&&e.outpoint.index===index);
   ...
   return {network,covenantId,transactionId,index,programHex,address:addressText,valueSompi:String(entry.amount),scriptPublicKey:version.toString(16).padStart(4,'0')+scriptHex,blockDaaScore:String(entry.blockDaaScore),liveBindingVerified:true,contractSemanticsVerified:false};
  },network);
