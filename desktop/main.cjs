...
async function walletUi(event,method,args={}){
  ...
  switch(method){
    case 'status': return {storageWarning:wallets?.warning||null,build:require('./build-info.cjs').buildInfo(),walletId:wallets?.id,wallets:wallets?await wallets.list():[],exists:await vault.exists(),locked:vault.locked,accounts:vault.accounts(kaspaService.network),kaspaNetwork:kaspaService.network,networks:NETWORKS,network:evm.network};
    case 'wallet-rename':{
...
ipcMain.handle('dapp-request',async(event,{family,method,params})=>{
  try{
    ...
    if((family==='kaspa'&&method==='disconnect')||(family==='evm'&&method==='wallet_revokePermissions')){
