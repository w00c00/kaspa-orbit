...
class Wallets{
 ...
 async save(){const temp=path.join(this.dir,crypto.randomBytes(16).toString('hex')+'.tmp');try{await fs.writeFile(temp,JSON.stringify({active:this.id,names:this.names}),{mode:0o600,flag:'wx'});await fs.rename(temp,path.join(this.dir,'index.json'));}finally{await fs.rm(temp,{force:true});}}
 ...
 async select(id){if(!(await this.list()).some(w=>w.id===id))throw Error('Wallet not found');this.vault.lock();this.id=id;this.vault=new Vault(this.file(id));await this.save();}
 ...
 async rename(name){this.names[this.id]=this.name(name);await this.save();}
}
