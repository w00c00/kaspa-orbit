...
test('KRC20 balance preserves integer precision and pagination',async()=>{
 ...
 assert.equal(observed.pathname,`/v1/krc20/address/${address}/tokenlist`);
 assert.equal(observed.pathname.includes('%3A'),false,'Kasplex requires a literal address prefix colon');
